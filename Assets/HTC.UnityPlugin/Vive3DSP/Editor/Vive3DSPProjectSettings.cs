    ...
    public class Vive3DSPProjectSettings : ScriptableObject, ISerializationCallbackReceiver
    {
        ...
        public static string defaultAssetPath
        {
            get
            {
                ...
                {
                    var ms = MonoScript.FromScriptableObject(CreateInstance<Vive3DSPProjectSettings>());
                    var msPath = AssetDatabase.GetAssetPath(ms);
                    s_defaultAssetPath = System.IO.Path.ChangeExtension(msPath.ToString(), "asset");
                }
        ...
        public static void Load(string path = null)
        {
            ...
            if ((s_instance = AssetDatabase.LoadAssetAtPath<Vive3DSPProjectSettings>(path)) == null)
            {
